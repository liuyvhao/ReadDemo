...
import kotlinx.android.synthetic.main.fragment_martialarts.view.*
 ...
 * 武侠修真
 */
class MartialArtsFragment : Fragment(), OnItemClickLitener {
    private var v: View? = null
    private var bookLists = ArrayList<BookList>()
    private var adapter: BookListAdapter? = null

    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        v = inflater.inflate(R.layout.fragment_martialarts, container, false)
        initView()
        initData()
        return v
    }
    ...
    private fun initData() {
        ...
        doAsync {
            var doc = Jsoup.connect(Head.urlHead + "/modules/article/articlelist.php?class=2").get()
            uiThread {
