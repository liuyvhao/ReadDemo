...
import kotlinx.android.synthetic.main.fragment_fantasy.view.*
 ...
 * 玄幻魔法
 */
class FantasyFragment : Fragment(), OnItemClickLitener {
    private var v: View? = null
    private var bookLists = ArrayList<BookList>()
    private var adapter: BookListAdapter? = null

    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        v = inflater.inflate(R.layout.fragment_fantasy, container, false)
        initView()
        initData()
        return v
    }
    ...
    private fun initData() {
        ...
        doAsync {
            var doc = Jsoup.connect(Head.urlHead + "/modules/article/articlelist.php?class=1").get()
            uiThread {
